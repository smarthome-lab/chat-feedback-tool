    ...
    <div class="content">
      ...
      <div>
        ...
        <sui-card-group :items-per-row="columns" stackable>
          <feedback-card v-for="feedback in feedbacks.data" v-bind:key=feedback v-bind:feedback=feedback :patchFeedback=patchFeedback />
        </sui-card-group>
      ...
      feathersClient
        ...
        .find({
          query: {
            ...
            tags:
              this.searchInput !== ''
                ? { $contains: this.searchInput.split(', ') }
                : undefined
          }
        })
        .then(feedback => {
